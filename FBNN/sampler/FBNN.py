...
class FBNN:
    """
    Fast Bayesian Neural Netwotk(FBNN) algorithms
    ----------------------------------------------------------------------
...
#             h_adpt['h']=self._init_h()
    ...
    def FBNNpCN(self):
        """
        preconditioned Crank-Nicolson under FBNN framework
        """
        logwt=0
        # initialization
        q=self.q.copy()
    ...
    def simple_mcmc(self):
        """
        Simple MCMC proposal and acceptance step under the FBNN framework.
        """
        logwt = 0
    
        # Current state and its log likelihood
        current_q = self.q
        current_ll, _, _, _ = self.geom(current_q)

        # Generate a proposal according to a simplified Crank-Nicolson scheme
        v = self.randv()
        proposal_q = ((1 - self.h/4) * current_q + np.sqrt(self.h) * v) / (1 + self.h/4)

        # Update geometry for the proposal
        proposal_ll, _, _, _ = self.geom(proposal_q)

        # Metropolis test
        logr = proposal_ll - current_ll + (logwt if self.vol_wts == 'adjust' else 0)
        ...
        # Return accept indicator, log ratio, and log weight
        return acpt, logr, logwt

    
    
    def compute_stochastic_gradient(current_q):
        """
        Compute the stochastic gradient of the log-likelihood with respect to the model parameters.

        Parameters:
            current_q: Current state of the parameters.

        Returns:
            grad: Stochastic gradient vector.
        """
        # You should implement your gradient computation logic here.
        # This could involve calculating gradients based on a subset of your data or other stochastic techniques.
        # For example, if you have a log-likelihood function log_likelihood(current_q, data), you might do something like this:

        # Randomly sample a subset of your data (mini-batch)
        mini_batch = random_sample_mini_batch(data)

        # Calculate the gradient of the log-likelihood with respect to the current_q using the mini-batch
        grad = gradient_of_log_likelihood(current_q, mini_batch)

        return grad

    def sghmc_mcmc(self):
        """
        SGHMC (Stochastic Gradient Hamiltonian Monte Carlo) under the FBNN framework.
        """
        logwt = 0
        ...
        noise = np.random.normal(0, np.sqrt(2 * self.h))

        # Compute the stochastic gradient (You need to implement this function)
        grad = compute_stochastic_gradient(current_q)

        # Update momentum
        current_q = current_q + self.h * np.dot(self.mass_matrix, grad) + noise

        # Calculate the log-likelihood for the new state
        proposal_ll, _, _, _ = self.geom(current_q)
    ...
    def FBNNinfMALA(self):
        """
        infinite dimensional Metropolis Adjusted Langevin Algorithm under FBNN framework
        """
        logwt=0
        # initialization
        q=self.q.copy()
    ...
    def FBNNinfHMC(self):
        """
        infinite dimensional Hamiltonian Monte Carlo under FBNN framework
        """
        logwt=0
        # initialization
        q=self.q.copy()
...
#             q=rot.real; v=rot.imag
    ...
    def FBNNinfmMALA(self):
        """
        dimension-reduced infinite dimensional manifold MALA under FBNN framework
        """
        logwt=0
        # initialization
        q=self.q.copy()
    ...
    def FBNNinfmHMC(self):
        """
        dimension-reduced infinite dimensional manifold HMC under FBNN framework
        """
        logwt=0
        # initialization
        q=self.q.copy()
